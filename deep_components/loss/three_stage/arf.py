...
class LRelaxLoss(LossHelperBase):
    def loss_graph(self):
        ...
        loss_sample_wise = torch.mean(-torch.log(target_permutation_matrix + 1e-6) * target_label_matrix * self.label_infos['label_mask'], dim=-1)
...
class ARFV1Loss(LossHelperBase):
    def __init__(self, name, label_infos, model_outputs, loss_conf, logger, use_name_as_scope=True, is_debug=False,
        ...
        super(ARFV1Loss, self).__init__(name=name, label_infos=label_infos, model_outputs=model_outputs,
                                      loss_conf=loss_conf,
...
def compute_arf_loss(inputs, rank_logits, prerank_logits, retrival_logits, device,
                        loss_model_rank=None,
    ...
    rerank_pos_mask, rerank_neg_mask, rank_neg_mask, coarse_mask, prerank_mask = mask_list
    ...
    if loss_type == 'arf_v2':
        loss_rank = ARFV2Loss(name='rank_model',
                            label_infos=label_infos,
                            model_outputs=model_outputs_dict,
                            loss_conf=rank_arf_loss_conf,
                            loss_model=loss_model_rank,
                            logger=logger,
        ...
        loss_rank = ARFV1Loss(name='rank_model',
                            label_infos=label_infos,
                            model_outputs=model_outputs_dict,
                            loss_conf=rank_arf_loss_conf,
                            loss_model=loss_model_rank,
    ...
    rank_loss = loss_rank.get_loss('total_loss')
    ...
    if loss_type == "arf_v2":
        loss_prerank = ARFV2Loss(name='prerank_model',
                               label_infos=label_infos,
                               model_outputs=model_outputs_dict,
                               loss_conf=prerank_arf_loss_conf,
        ...
        loss_prerank = ARFV1Loss(name='prerank_model',
                               label_infos=label_infos,
                               model_outputs=model_outputs_dict,
                               loss_conf=prerank_arf_loss_conf,
                               loss_model=loss_model_prerank,
    ...
    if loss_type == "arf_v2":
        loss_retrival = ARFV2Loss(name='retrival_model',
                                label_infos=label_infos,
                                model_outputs=model_outputs_dict,
                                loss_conf=retrival_arf_loss_conf,
        ...
        loss_retrival = ARFV1Loss(name='retrival_model',
                                label_infos=label_infos,
                                model_outputs=model_outputs_dict,
                                loss_conf=retrival_arf_loss_conf,
                                loss_model=loss_model_retrival,
    ...
    outputs = {
        "total_loss": rank_loss + prerank_loss + retrival_loss,
        "rank_loss": rank_loss,
