...
def compute_lambda_loss(labels_raw, pred_raw, device, smooth_fraction, topn):
    gain_fn = lambda label: label
    rank_discount_fn = lambda rank: 1 / torch.log(rank + 1)
    pred_raw = pred_raw.to(device)
    # Compute ranks
    ranks_idx = torch.argsort(torch.argsort(pred_raw, dim=1, descending=True), dim=1) + 1
...
def compute_fs_lambdaloss(inputs, rank_logits, prerank_logits, retrival_logits, device, smooth_fraction, topn, is_debug=False):
    # topk,sigma,label_type,epsilon
    rank_index_list = [tensor.to(device) for tensor in inputs[-5:]]
    all_rank_logits = torch.cat([logits.to(device) for logits in rank_logits], dim=1)
    all_prerank_logits = torch.cat([logits.to(device) for logits in prerank_logits], dim=1)
    all_retrival_logits = torch.cat([logits.to(device) for logits in retrival_logits], dim=1)
    all_rank_index = torch.cat([rank_index.to(device) for rank_index in rank_index_list], dim=1)

    rank_loss = compute_lambda_loss(all_rank_index, all_rank_logits, device, smooth_fraction, topn)
    retrival_loss = compute_lambda_loss(all_rank_index, all_retrival_logits, device, smooth_fraction, topn)
    prerank_loss = compute_lambda_loss(all_rank_index, all_prerank_logits, device, smooth_fraction, topn)

    if is_debug: print("compute_fsltr_lambda_all_by_rank retrival_loss=%s, prerank_loss=%s, rank_loss=%s" % (
    retrival_loss, prerank_loss, rank_loss))
    loss = retrival_loss + prerank_loss + rank_loss
