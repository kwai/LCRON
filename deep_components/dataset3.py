...
class Rank_Train_All_BY_RERANK_Dataset(Dataset):
  def __init__(
          self,
          path_to_csv,
          seq_len,
    ...
    idx_begin = time.time()
    self.rerank_index_df = self.get_rerank_index_pd(raw_df)
  ...
  def search_rerank_index_list(self, request_id, video_ids, default_v = 500):
    rerank_index_ls = []
    for video_id in video_ids:
      key_to_lookup = (request_id, video_id)
      val = self.rerank_index_df.get(key_to_lookup)
      if val is None:
        val = default_v
        self.search_miss_num+=1
      else:
        self.search_hit_num+=1
      rerank_index_ls.append(val[0])

    rerank_index_ls = np.asarray(rerank_index_ls)
    return rerank_index_ls

  def get_rerank_index_pd(self, df):
    df = df[df["rerank_pos"] == 1]
    grouped = df.groupby(["request_id", "video_id"])["rerank_index"]
    print("get_rerank_index_pd. df.head=%s"%df.head(100))
    print("get_rerank_index_pd. grouped.head=%s"%grouped.head(100))
  ...
  def __getitem__(self, idx):
    ...
    max_sz = 10
    rerank_pos_photos, rerank_pos_mask, rerank_pos_rank_index = self.initialize_photo_data(max_sz, default_rank_index=0)

    if 'rerank_pos' in self.request_dict[request_id] and len(self.request_dict[request_id]['rerank_pos']) > 0:
      rerank_pos_flow_photos = self.request_dict[request_id]['rerank_pos'].copy()[:max_sz]
      self.fill_photo_data(rerank_pos_photos, rerank_pos_mask, rerank_pos_flow_photos, max_photos=max_sz)
      rerank_index_ls = self.search_rerank_index_list(request_id, rerank_pos_flow_photos[:, 0])
      rerank_pos_rank_index[:len(rerank_pos_flow_photos)] = rerank_index_ls

    rerank_neg_photos, rerank_neg_mask, rerank_neg_rank_index = self.initialize_photo_data(max_sz, default_rank_index=22+self.rank_offset)
    if 'rerank_neg' in self.request_dict[request_id] and len(self.request_dict[request_id]['rerank_neg']) > 0:
      rerank_neg_flow_photos = self.request_dict[request_id]['rerank_neg'].copy()[:max_sz]
      self.fill_photo_data(rerank_neg_photos, rerank_neg_mask, rerank_neg_flow_photos, max_photos=max_sz)

    rank_neg_photos, rank_neg_mask, rank_neg_rank_index = self.initialize_photo_data(max_sz, default_rank_index=23+self.rank_offset*2)
    ...
    coarse_neg_photos, coarse_neg_mask, coarse_neg_rank_index = self.initialize_photo_data(max_sz, default_rank_index=24+self.rank_offset*3)
    ...
    prerank_neg_photos, prerank_neg_mask, prerank_neg_rank_index = self.initialize_photo_data(max_sz, default_rank_index=25+self.rank_offset*4)
    ...
    return request_ts_wday, request_ts_hour, request_ts_min, \
      ...
      rerank_pos_photos, \
      rerank_neg_photos, rank_neg_photos, coarse_neg_photos, prerank_neg_photos, \
      rerank_pos_mask, \
      rerank_neg_mask, rank_neg_mask, coarse_neg_mask, prerank_neg_mask, \
      max_rank_index - rerank_pos_rank_index,\
      max_rank_index - rerank_neg_rank_index, max_rank_index - rank_neg_rank_index, max_rank_index - coarse_neg_rank_index, max_rank_index - prerank_neg_rank_index
  ...
  def fill_photo_data(self, photos, mask, flow_photos, max_photos):
    ...
    if n_photos > 0:
      ...
      photos[:n_photos, 0] = sampling_flow_photos[:, 0] + 1  # video_id
      photos[:n_photos, 1] = sampling_flow_photos[:, 1] + 1  # author_id
      photos[:n_photos, 2] = sampling_flow_photos[:, 3] + 1  # cate_two
      photos[:n_photos, 3] = sampling_flow_photos[:, 6]  # cate_one
      photos[:n_photos, 4] = sampling_flow_photos[:, 4] + 1  # upload_type
